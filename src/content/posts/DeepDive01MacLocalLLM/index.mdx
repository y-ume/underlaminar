...
```expressive-code language="txt" title="➤ 决策" ins={1}
选择：MLX
✓ 生态前景好
✓ 低层优化更好
✓ 当前不涉及多模型并发场景
...
```expressive-code language="txt" title="➤ 思考" ins={1}
使用 uv 管理 Python 环境
✓ 隔离简单又干净
✓ 装环境恐惧症患者的福音
```

```expressive-code language="bash" title="➤ 实施步骤"
# 安装 uv
...
```expressive-code language="bash" title="➤ 方式 1：活动监视器"
# 打开活动监视器
# 肉眼观察 python 进程
```

```expressive-code language="bash" title="➤ 方式 2：asitop（推荐）"
