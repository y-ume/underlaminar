...
```expressive-code language="bash" title="➤ V2Ray 安装"
ssh root@{服务器IP}
...
```expressive-code language="bash" title="➤ 安装 zsh 和 Oh My Zsh"
...
```expressive-code language="bash" title="~/.zshrc 关键配置"
# 目录与清理别名
alias .="cd .."
alias cl="clear"
...
```expressive-code language="bash" title="mystyle.zsh-theme"
...
```expressive-code language="bash" title="➤ nvm 和 Node.js 安装"
...
```expressive-code language="bash" title="➤ 启动 Claude Code"
cc
```
...
```expressive-code language="bash" title="➤ frps.toml（阿里云）"
bindPort = 7000
auth.token = "复杂密钥1"
```
...
```expressive-code language="bash" title="➤ frpc.toml（开发机）"
serverAddr = "公网IP"
serverPort = 7000
auth.token = "复杂密钥1"
...
```expressive-code language="bash" title="➤ frpc.toml（本机）"
serverAddr = "公网IP"
serverPort = 7000
auth.token = "复杂密钥1"
...
```expressive-code language="bash" title="➤ 启动与连接"
# 开发机启动 FRPC
frpc -c frpc.toml
...
```expressive-code language="bash" title="➤ 服务器：创建账号"
...
```expressive-code language="bash" title="➤ 本机：生成密钥对"
ssh-keygen -t ed25519 -f ~/.ssh/{key_name} -C "name"
```

本地 `~/.ssh/config`:

```expressive-code language="bash" title="~/.ssh/config"
Host {取个名字}
  HostName 127.0.0.1
  Port 6000
  User {newuser}
...
```expressive-code language="bash" title="➤ 服务器：注入公钥"
mkdir -p ~/.ssh
chmod 700 ~/.ssh
...
```expressive-code language="bash" title="/etc/ssh/sshd_config 修改"
PasswordAuthentication no
ChallengeResponseAuthentication no
PubkeyAuthentication yes
```

```expressive-code language="bash" title="➤ 服务器：重启 SSH"
